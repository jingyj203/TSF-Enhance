...
def loss_proxy(nb_classes,D,reduce=False):
    loss = 0
    for i in range(nb_classes):
        fenmu = 0
        for j in range(nb_classes):
            fenmu = fenmu + torch.exp(-D[i][j])
        fenmu = fenmu + 0.00015
        loss = loss + torch.log(fenmu)
    return loss/float(nb_classes)
...
class ProxyNCA_prob(torch.nn.Module):
    ...
    def forward(self, X, T):
        P = self.proxies
        # in the paper, value "9": scale = sart(1/(1/9)) = sqrt(9) = 3
        # we need to apply sqrt because the pairwise distance is calculated as norm^2
